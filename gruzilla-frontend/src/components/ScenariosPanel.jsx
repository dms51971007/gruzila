...
export default function ScenariosPanel({ baseUrl }) {
  const [dir, setDir] = useState("scenarios");
  ...
  const loadList = async () => {
    const response = await callApi("/api/v1/scenarios/list", { dir });
    if (!response) return;
  ...
  const readSelected = async (selectedPath = path) => {
    ...
    const response = await callApi("/api/v1/scenarios/read", { dir, path: selectedPath });
    if (!response) return;
    const data = extractCliData(response.payload);
  ...
  return (
    <Stack spacing={2}>
      <Card>
        <CardContent>
          <Stack spacing={2}>
            <Typography variant="h6">Сценарии: список и редактирование</Typography>
            ...
            <Grid container spacing={2}>
                  ...
                  <CardContent>
                    <Stack spacing={1.5}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        <Typography variant="subtitle1">Список сценариев</Typography>
              ...
              <Grid size={{ xs: 12, md: 7 }}>
                <Card variant="outlined" sx={{ borderRadius: 3 }}>
                  <CardContent>
                    <Stack spacing={1.5}>
                      <Typography variant="subtitle1">Редактор YAML</Typography>
                      <TextField
                        multiline
                        minRows={16}
                        label="YAML Content (for create/update)"
                        value={content}
                        onChange={(e) => setContent(e.target.value)}
                      />
                      <Stack direction="row" spacing={1} flexWrap="wrap">
                        <Button disabled={loading} variant="contained" onClick={() => callApi("/api/v1/scenarios/create", { dir, path, content })}>
                          Create
                        </Button>
                        <Button disabled={loading} variant="contained" onClick={() => callApi("/api/v1/scenarios/update", { dir, path, content })}>
                          Update
                        </Button>
                        <Button
                          disabled={loading}
                          color="error"
                          variant="outlined"
                          onClick={() => callApi("/api/v1/scenarios/delete", { dir, path, yes: true })}
                        >
                          Delete
                        </Button>
                      </Stack>
